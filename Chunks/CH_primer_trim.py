...
3. CH region coding strand! (fasta)
...
def trim_5(seq, fwd, m):
	...
	for f in fwd:
		...
		if len(seq) >= n:
			...
			#print(alignment_penalty(seq[:n], f))
...
def trim_3(seq, rev, m):
	...
	for r in rev:
		...
		if len(seq) >= n:
			...
			#print(alignment_penalty(seq[-n:], r))
...
def main():
    ### parameters
	random_nuleotides_3 = 4
	random_nuleotides_5 = 4
	ECBC_length = 12
	allowed_mismatches = 3
	minimal_trimmed_nucleotides = 0
	maximal_trimmed_nucleotides = 1000
	...
	file_name = file_name.replace("_panda.fasta", "_var.fasta")
	...
	fwd, rev = [str(s.seq) for s in fwd], [str(s.seq) for s in rev]
	...
	for fasta in sequences:
        ...
        ### trim random nucleotides at 5' end of sequences for IgG
		if 'IgG' in sys.argv[1]:
			sequence = sequence[random_nuleotides_5:]
		
		### trim random nucleotides and ECEC at 3' end for all other sequences
		else:
			sequence = sequence[:-(random_nuleotides_3 + ECBC_length)]

        ### remove fwd primers and CH region
		sequence, H_f = trim_5(sequence, fwd, allowed_mismatches)
		sequence, H_r = trim_3(sequence, rev, allowed_mismatches)
        
        ### filter output
		if minimal_trimmed_nucleotides <= length - len(sequence) <= maximal_trimmed_nucleotides:
			print(">%s|%sL-%sV-%sCH" % (name, H_f, len(sequence), H_r))
			print(sequence)
			
			out = ">%s|%sL-%sV-%sCH" % (name, H_f, len(sequence), H_r) + '\n' + sequence + '\n'
			file.write(out)
